...
shop_managers = db.Table('shop_managers',
    db.Column('user_id', db.Integer, db.ForeignKey('users.id'), primary_key=True), # Corrected ForeignKey to 'users.id'
    db.Column('shop_id', db.Integer, db.ForeignKey('merchants.id'), primary_key=True) # Corrected ForeignKey to 'merchants.id'
)
...
class Merchant(db.Model):
    ...
    paybill_number = db.Column(db.String(20), unique=True)
    pochi_number = db.Column(db.String(20), unique=True)
    qr_code_url = db.Column(db.String(255))
    created_at = db.Column(db.TIMESTAMP, default=datetime.utcnow, nullable=False)
...
class Transaction(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    agent_id = db.Column(db.Integer, db.ForeignKey('mpesa_agents.id'), nullable=True) # Optional Foreign Key to M-Pesa Agents table
    merchant_id = db.Column(db.Integer, db.ForeignKey('merchants.id'), nullable=True) # Optional Foreign Key to Merchants table
...
class QRCode(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    entity_id = db.Column(db.Integer, nullable=False) # ID of the linked entity (Agent or Merchant)
    identity_type = db.Column(db.Enum(QRCodeEntityType), nullable=False) # Type of entity (Agent, Till, Paybill, Pochi)
    qr_code_url = db.Column(db.String(255))
    created_at = db.Column(db.TIMESTAMP, default=datetime.utcnow, nullable=False)
    agent_id = db.Column(db.Integer, db.ForeignKey('mpesa_agents.id'), nullable=True) # Foreign Key to Agents (for Agent QR codes)
    merchant_till_id = db.Column(db.Integer, db.ForeignKey('merchants.id'), nullable=True) # Foreign Key to Merchants (for Till QR codes)
    merchant_paybill_id = db.Column(db.Integer, db.ForeignKey('merchants.id'), nullable=True) # Foreign Key to Merchants (for Paybill QR codes)
    merchant_pochi_id = db.Column(db.Integer, db.ForeignKey('merchants.id'), nullable=True) # Foreign Key to Merchants (for Pochi QR codes)
